...
type GuardPill = { tone: "neutral" | "warn" | "danger"; label: string; detail?: string };
...
const ReviewerActionBar: React.FC<{
  ...
  onReject,
    ...
    const renderGuardPill = (p: GuardPill, idx: number) => (
      <span
        key={`${p.label}-${idx}`}
        className={cx("cb-reviewer-guard-pill", `cb-reviewer-guard-pill--${p.tone}`)}
      ...
      <div className="cb-reviewer-guard-row">
        ...
        <span
          className={cx(
            ...
            row.allowed ? "cb-reviewer-guard-state--ok" : "cb-reviewer-guard-state--no"
          )}
        >
          {row.allowed ? "가능" : "불가"}
        </span>
        <div className="cb-reviewer-guard-pills">{row.pills.map(renderGuardPill)}</div>
      </div>
    );
    ...
    return (
      <div className="cb-reviewer-actionbar">
        <div className="cb-reviewer-actionbar-left">
          {!actionGuard ? (
            <span className="cb-reviewer-muted">좌측 목록에서 검토할 항목을 선택하세요.</span>
          ) : (
